...
# -------------------------
# Locate _GDS.yaml safely (with auto-generation)
# -------------------------
...
if GDS_YAML === nothing
    @warn "No _GDS.yaml found. Generating minimal _GDS.yaml..."
    
    GDS_YAML = joinpath(BUILD_DIR, "$(DEPLOYMENT_NAME)_GDS.yaml")
    
    open(GDS_YAML, "w") do io
        println(io, """\
# Auto-generated minimal GDS YAML
...
""")
    ...
    println("✅ Generated minimal GDS YAML at: $GDS_YAML")
end

println("ℹ️ Using GDS YAML: $GDS_YAML")
